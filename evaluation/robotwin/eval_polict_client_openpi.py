...
robowin_root = Path(os.environ.get(
    "ROBOTWIN_ROOT",
    "/mnt/afs/intern/manlichen/ivan/zhoujunl/Wam_Speed_up/RoboTwin",
))
...
from evaluation.robotwin.specverify_client_policy import RiskRouterClientPolicy, SpecVerifyClientPolicy
...
def main(usr_args):
    ...
    st_seed = int(usr_args.get("start_seed")) if usr_args.get("start_seed") is not None else 10000 * (1 + seed)
    suc_nums = []
    test_num = usr_args["test_num"]

    if usr_args.get("specverify_enable", False):
        policy_mode = usr_args.get("specverify_policy_mode", "rtvla")
        if policy_mode == "risk_router":
            model = RiskRouterClientPolicy(
                host=usr_args.get("host", "0.0.0.0"),
                draft_port=int(usr_args["draft_port"]),
                teacher_port=int(usr_args["teacher_port"]),
                threshold=float(usr_args.get("specverify_threshold", 0.20)),
                tau_timesteps=usr_args.get("specverify_tau", [150.0, 300.0]),
                teacher_cache_mode=usr_args.get("specverify_teacher_cache_mode", "lazy_reference"),
                risk_low=float(usr_args.get("risk_router_low", 0.45)),
                risk_high=float(usr_args.get("risk_router_high", 0.80)),
                risk_verify_mode=usr_args.get("risk_router_verify_mode", "medium"),
                risk_delta_ref=float(usr_args.get("risk_router_delta_ref", 0.25)),
                risk_mean_delta_ref=float(usr_args.get("risk_router_mean_delta_ref", 0.12)),
            ...
            model = SpecVerifyClientPolicy(
            host=usr_args.get("host", "0.0.0.0"),
            draft_port=int(usr_args["draft_port"]),
            teacher_port=int(usr_args["teacher_port"]),
            pf_interval=int(usr_args.get("specverify_pf", 2)),
            threshold=float(usr_args.get("specverify_threshold", 0.15)),
            tau_timesteps=usr_args.get("specverify_tau", [150.0, 300.0]),
            teacher_cache_mode=usr_args.get("specverify_teacher_cache_mode", "sync"),
            prime_draft_on_teacher_full=bool(usr_args.get("specverify_prime_draft_on_teacher_full", True)),
            phase_mode=usr_args.get("specverify_phase_mode", "fallback"),
            phase_threshold_scale=float(usr_args.get("specverify_phase_threshold_scale", 0.5)),
            log_path=usr_args.get("specverify_log"),
            )
    else:
...
def parse_args_and_config():
    ...
    parser.add_argument("--specverify_enable", action="store_true")
    parser.add_argument(
        "--specverify_policy_mode",
        choices=["rtvla", "risk_router"],
        default="rtvla",
    )
    parser.add_argument("--draft_port", type=int, default=None)
    parser.add_argument("--teacher_port", type=int, default=None)
    parser.add_argument("--specverify_log", type=str, default=None)
    parser.add_argument("--specverify_pf", type=int, default=2)
    parser.add_argument("--specverify_threshold", type=float, default=0.15)
    parser.add_argument("--specverify_tau", nargs="+", type=float, default=[150.0, 300.0])
    parser.add_argument(
    ...
    parser.add_argument(
        "--specverify_disable_prime_draft_on_teacher_full",
        action="store_true",
        help="Do not run the draft server on teacher-full action requests.",
    )
    parser.add_argument(
        "--specverify_phase_mode",
        choices=["fallback", "tighten", "ignore"],
        default="fallback",
        help="How to handle gripper-switch phase risk: original fallback, tighten verify threshold, or ignore.",
    )
    parser.add_argument(
        "--specverify_phase_threshold_scale",
        type=float,
        default=0.5,
        help="Multiplier for --specverify_threshold when --specverify_phase_mode=tighten.",
    )
    parser.add_argument("--risk_router_low", type=float, default=0.45)
    parser.add_argument("--risk_router_high", type=float, default=0.80)
    parser.add_argument("--risk_router_verify_mode", choices=["off", "medium"], default="medium")
    parser.add_argument("--risk_router_delta_ref", type=float, default=0.25)
    ...
    parser_updates = {
        ...
        "specverify_enable": args.specverify_enable,
        "specverify_policy_mode": args.specverify_policy_mode,
        "draft_port": args.draft_port,
        "teacher_port": args.teacher_port,
        "specverify_log": args.specverify_log,
        "specverify_pf": args.specverify_pf,
        "specverify_threshold": args.specverify_threshold,
        "specverify_tau": args.specverify_tau,
        "specverify_teacher_cache_mode": args.specverify_teacher_cache_mode,
        "specverify_prime_draft_on_teacher_full": not args.specverify_disable_prime_draft_on_teacher_full,
        "specverify_phase_mode": args.specverify_phase_mode,
        "specverify_phase_threshold_scale": args.specverify_phase_threshold_scale,
        "risk_router_low": args.risk_router_low,
        "risk_router_high": args.risk_router_high,
        "risk_router_verify_mode": args.risk_router_verify_mode,
        "risk_router_delta_ref": args.risk_router_delta_ref,
    ...
    if "--overrides" in argv_before_overrides:
        argv_before_overrides = argv_before_overrides[:argv_before_overrides.index("--overrides")]
    explicit_flags = {arg for arg in argv_before_overrides if arg.startswith("--")}
    flag_for_key = {
        "host": "--host",
        "port": "--port",
        "save_root": "--save_root",
        ...
        "action_guidance_scale": "--action_guidance_scale",
        "test_num": "--test_num",
        "specverify_enable": "--specverify_enable",
        "specverify_policy_mode": "--specverify_policy_mode",
        "draft_port": "--draft_port",
        "teacher_port": "--teacher_port",
        "specverify_log": "--specverify_log",
        "specverify_pf": "--specverify_pf",
        "specverify_threshold": "--specverify_threshold",
        "specverify_tau": "--specverify_tau",
        "specverify_teacher_cache_mode": "--specverify_teacher_cache_mode",
        "specverify_prime_draft_on_teacher_full": "--specverify_disable_prime_draft_on_teacher_full",
        "specverify_phase_mode": "--specverify_phase_mode",
        "specverify_phase_threshold_scale": "--specverify_phase_threshold_scale",
        "risk_router_low": "--risk_router_low",
        "risk_router_high": "--risk_router_high",
        "risk_router_verify_mode": "--risk_router_verify_mode",
        "risk_router_delta_ref": "--risk_router_delta_ref",
    ...
    if config["specverify_enable"]:
        if config["draft_port"] is None or config["teacher_port"] is None:
            raise ValueError("--specverify_enable requires --draft_port and --teacher_port")
