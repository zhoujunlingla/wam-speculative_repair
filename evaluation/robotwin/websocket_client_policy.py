...
class WebsocketClientPolicy:
    ...
    def __init__(self, host: str = "0.0.0.0", port: Optional[int] = None, api_key: Optional[str] = None) -> None:
        ...
        self._latency_log = os.environ.get("WANVA_LATENCY_LOG")
        if self._latency_log:
            Path(self._latency_log).parent.mkdir(parents=True, exist_ok=True)
    ...
    def infer(self, obs: Dict) -> Dict:  # noqa: UP006
        ...
        self._log_latency(obs, elapsed)
        if isinstance(response, str):
            # we're expecting bytes; if the server sends a string, it's an error.
            raise RuntimeError(f"Error in inference server:\n{response}")
        return unpackb(response)

    def _log_latency(self, obs: Dict, elapsed: float) -> None:
        ...
        elif obs.get("verify_action", False):
            request_type = "verify_action"
        else:
            request_type = "action"
        record = {
        ...
        with Path(self._latency_log).open("a") as f:
