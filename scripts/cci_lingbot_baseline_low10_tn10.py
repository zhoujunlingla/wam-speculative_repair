#!/usr/bin/env python3
"""Launch pure LingBot-VA RobotWin clean low10 evaluation on cci.

This runner intentionally disables SpecVerify. It starts one LingBot-VA
posttrain server and runs the requested RoboTwin clean tasks through the normal
websocket client policy. Artifacts stay under the zhoujunl project roots.
"""
...
def start_lingbot_server(
    run_root: Path, *, gpu: int, port: int, master_port: int, config_name: str
) -> subprocess.Popen:
    log_path = run_root / "logs" / f"lingbot_server_g{gpu}.log"
    ...
    cmd = [
        ...
        config_name,
        "--save_root",
        str(run_root / "server" / "lingbot"),
        "--port",
        str(port),
    ]
    print(f"[launcher] start pure LingBot server gpu={gpu} port={port} config={config_name}", flush=True)
...
def summarize(
    ...
    *,
    ...
    ref = sum(TEACHER_CLEAN_RATE[t] for t in TASKS) / len(TASKS)
    summary = {
        ...
        "model": "lingbot-va-posttrain-robotwin",
        "config_name": config_name,
        "video_steps": video_steps,
        "action_steps": action_steps,
        "tasks": TASKS,
        "test_num_per_task": 10,
        "success": success,
        "total": total,
        "success_rate": rate,
        "teacher_low10_mean_rate": ref,
        "delta_vs_teacher_low10_mean": rate - ref,
    ...
    for root in (run_root, result_root):
        ...
        with (root / "per_task_success.csv").open("w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f,
                fieldnames=["task", "succ_num", "total_num", "succ_rate", "teacher_clean_rate", "delta_vs_teacher", "status"],
            )
            writer.writeheader()
            writer.writerows(rows)
        lines = [
            "# Pure LingBot-VA Low10 TN10 Baseline",
            "",
            f"- Start: {start_time}",
            f"- End: {end_time or 'RUNNING'}",
            f"- Experiment: {content}",
            f"- Current success: {success}/{total} = {rate * 100:.2f}%",
            f"- Reference low10 mean: {ref * 100:.2f}%",
            f"- Delta: {(rate - ref) * 100:+.2f} pp",
            "",
        ...
        for row in rows:
            lines.append(
                ...
                f"{float(row['succ_rate']) * 100:.2f}% | {float(row['teacher_clean_rate']) * 100:.2f}% | "
...
def main() -> None:
    ...
    parser.add_argument("--server-gpu", type=int, required=True)
    parser.add_argument("--client-gpu", type=int, required=True)
    parser.add_argument("--port", type=int, required=True)
    parser.add_argument("--master-port", type=int, required=True)
    parser.add_argument("--server-warmup-sec", type=int, default=300)
    parser.add_argument("--test-num", type=int, default=10)
    parser.add_argument("--tasks", nargs="+", default=TASKS)
    parser.add_argument("--config-name", default="robotwin_lingbot_posttrain")
    parser.add_argument("--video-steps", type=int, default=25)
    parser.add_argument("--action-steps", type=int, default=50)
    parser.add_argument("--ckpt-setting", default="LingBot-VA-posttrain-RoboTwin-v25a50-baseline")
    parser.add_argument("--experiment-content", default=None)
    ...
    content = args.experiment_content or (
        f"Pure LingBot-VA posttrain v{args.video_steps}/a{args.action_steps} "
        "baseline on RoboTwin clean low10 TN10; SpecVerify disabled."
    )
    start_time = datetime.now().isoformat(timespec="seconds")
    for root in (run_root, result_root):
        (root / "command.sh").write_text(" ".join(sys.argv) + "\n", encoding="utf-8")
        (root / "start_time.txt").write_text(start_time + "\n", encoding="utf-8")
    ...
    try:
        ...
        print(f"[launcher] warming server for {args.server_warmup_sec}s", flush=True)
