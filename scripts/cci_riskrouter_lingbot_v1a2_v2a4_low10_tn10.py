...
def specverify_metrics(log_path: Path) -> dict[str, object]:
    counts = {
        ...
        "phase_fallback": 0,
        "phase_tighten": 0,
        "phase_tighten_accept": 0,
        "phase_tighten_reject": 0,
        "verify_reject": 0,
        "draft_low_risk": 0,
    ...
    with log_path.open() as f:
        for line in f:
            ...
            if source == "draft":
                ...
                elif reason == "verify_reject_phase_tightened":
                    counts["verify_reject"] += 1
                    counts["phase_tighten"] += 1
    ...
    return {
        ...
        "rates": {
            ...
            "phase_tighten_rate": rate(counts["phase_tighten"]),
    ...
    *,
    ...
    for root in (run_root, result_root):
        ...
        lines = [
            ...
            "- Draft: LingBot-VA v1/a2 direct",
            "- Teacher/fallback: LingBot-VA v2/a4 direct",
            "- Router: cheap action-risk first, optional medium-risk action-flow verify",
            "- Runtime controls: no periodic full refresh; phase switch is a risk signal, not a hard fallback",
        ...
        for idx, task in enumerate(TASKS, start=1):
            ...
            rc, status = run_task(
                ...
                teacher_port=teacher_port,
                    ...
                    specverify_pf=args.specverify_pf,
                    specverify_threshold=args.specverify_threshold,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="fallback",
        help="How to handle gripper-switch phase risk.",
    )
    parser.add_argument(
        "--phase-threshold-scale",
        type=float,
        default=0.5,
        help="Threshold multiplier when --phase-mode=tighten.",
    )
    parser.add_argument(
        "--draft-config-name",
    ...
    parser.add_argument(
        ...
        help="sync updates teacher every cache update; lazy_reference flushes accumulated cache before teacher; stale_reference stores only latest cache and syncs it only before teacher action.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--risk-low", type=float, default=0.45)
    parser.add_argument("--risk-high", type=float, default=0.80)
    parser.add_argument("--risk-verify-mode", choices=["off", "medium"], default="medium")
