...
STAMP="$(date +%H%M%S)"
RUN_ID="${RUN_ID:-20260620_specverify_rtvla_low10_tn10_a800_8gpu_${STAMP}}"
...
cat > "$RESULT_ROOT/run_manifest.json" <<JSON
{
  "run_id": "$RUN_ID",
  "start_time": "$(date '+%F %T')",
  "experiment_content": "Realtime-VLA first-version SpecVerify on LingBot-VA: FlashWAM v1/a1 draft + LingBot v25/a50 teacher verifier/fallback, low10 clean TN10, 8-GPU sharded.",
  "gpu_policy": "Use GPU0-7 without killing or preempting unrelated processes. Each shard uses one server GPU for draft+teacher and one client GPU for RoboTwin.",
  "shards": [
    ...
    {"name": "shard2", "server_gpu": 4, "client_gpu": 5, "tasks": ["place_can_basket", "stack_bowls_three"]},
    {"name": "shard3", "server_gpu": 6, "client_gpu": 7, "tasks": ["open_microwave", "put_bottles_dustbin"]}
  ]
}
JSON
...
launch_shard shard0 0 1 36000 37000 hanging_mug press_stapler dump_bin_bigbin &
echo "$!" > "$RESULT_ROOT/logs/shard0.pid"
launch_shard shard1 2 3 36100 37100 turn_switch put_object_cabinet pick_diverse_bottles &
echo "$!" > "$RESULT_ROOT/logs/shard1.pid"
launch_shard shard2 4 5 36200 37200 place_can_basket stack_bowls_three &
echo "$!" > "$RESULT_ROOT/logs/shard2.pid"
launch_shard shard3 6 7 36300 37300 open_microwave put_bottles_dustbin &
...
summary = {
    ...
    "exit_status": status,
    "experiment_content": "Realtime-VLA first-version SpecVerify on LingBot-VA, 8-GPU sharded low10 clean TN10.",
    "success": success,
    "total": total,
    "success_rate": rate,
...
for root in (result_root, exp_root):
    ...
    lines = [
        "# SpecVerify RTVLA Low10 TN10 A800 8GPU Sharded",
        "",
